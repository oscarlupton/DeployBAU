...
$adtSession = @{
    ...
    AppScriptVersion = '1.4.0'
    AppScriptDate = '2026-05-12'
...
function Install-ADTDeployment
{
    ...
    #   - verifone_usb_drivers      - NOT included as not required
    #   - configure_rndis_service   - LinklyRndisService registration
    #   - optimise_win_pwr_settings - Disables sleep and stuff
    $linklyInstallArgs =
        '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART ' +
        '/TYPE="linkly_client" ' +
        '/TASKS="ems_client_service,ingenico_usb_drivers,configure_rndis_service,optimise_win_pwr_settings"'
    ...
    New-NetFirewallRule -DisplayName "Allow Linkly Eft1" -Direction Outbound -Program "$linklyInstallDir\Eftsrv.exe" -Action Allow
    New-NetFirewallRule -DisplayName "Allow Linkly Eft2" -Direction Inbound -Program "C:\Program Files (x86)\PC_EFT\Eftsrv.exe" -Action Allow
    New-NetFirewallRule -DisplayName "Allow Linkly Eft3" -Direction Outbound -Program "C:\Program Files (x86)\PC_EFT\EftClnt.exe" -Action Allow
    New-NetFirewallRule -DisplayName "Allow Linkly Eft4" -Direction Inbound -Program "C:\Program Files (x86)\PC_EFT\EftClnt.exe" -Action Allow
    ...
    # NoConfig_LinklySetup.exe handles everything except for connecting to Aspect
    if (-not (Test-Path -LiteralPath "$linklyInstallDir\EftClnt.exe")) {
